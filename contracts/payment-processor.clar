...
;; Process STX payment
(define-private (process-stx-payment (recipient principal) (amount uint))
...
;; Process token payment using a given fungible token contract
(define-private (process-token-payment (recipient principal) (amount uint) (token-contract principal))
  (let
    (
      (ft-transfer-result (contract-call? token-contract transfer amount tx-sender recipient none))
    )
    (if (is-ok ft-transfer-result)
        (ok true)
        ERR-PAYMENT-FAILED
    )
  )
...
(define-private (check-payment-threshold (user principal) (amount uint))
  (let
    (
      (settings (default-to 
                  { enabled: false, max-payment-threshold: u0, requires-approval-above-threshold: true }
...
(define-private (is-payment-due (subscription-id uint))
  (match (map-get? active-subscriptions { subscription-id: subscription-id })
    subscription
      (if (>= block-height (get next-payment-block subscription))
          (ok true)
          ERR-PAYMENT-NOT-DUE)
    ...
    subscription
      (begin
        (map-set active-subscriptions
          ...
          (merge subscription {
            next-payment-block: (+ (get next-payment-block subscription) (get payment-period subscription)),
...
(define-private (record-payment (subscription-id uint) (status (string-ascii 20)))
  (let
    (
      (payment-id (generate-payment-id))
      (subscription (unwrap! (map-get? active-subscriptions { subscription-id: subscription-id }) ERR-INVALID-SUBSCRIPTION))
    )
    (map-set payments
      { payment-id: payment-id, subscription-id: subscription-id }
      {
...
;; Get payment details
(define-read-only (get-payment-details (payment-id uint) (subscription-id uint))
  (map-get? payments { payment-id: payment-id, subscription-id: subscription-id })
)
...
(define-read-only (get-pending-approval (subscription-id uint) (payment-id uint))
  (map-get? pending-approvals { subscription-id: subscription-id, payment-id: payment-id })
)
...
(define-public (register-service (name (string-ascii 64)))
  (let
    (
      (service-id (generate-service-id))
    )
...
(define-public (create-subscription (service-id uint) (recipient principal) (payment-amount uint) 
                                    (payment-period uint) (token-contract (optional principal)))
  (let
    (
      (subscription-id (generate-subscription-id))
      (service (default-to { name: "", owner: tx-sender, active: false } 
                (map-get? subscription-services { service-id: service-id })))
    )
...
(define-public (configure-auto-payment (enabled bool) (max-payment-threshold uint) (requires-approval-above-threshold bool))
  (begin
...
;; Process a manual payment for a subscription
(define-public (make-payment (subscription-id uint))
  (let
    (
      (payment-due-result (is-payment-due subscription-id))
      (subscription (unwrap! (map-get? active-subscriptions { subscription-id: subscription-id }) ERR-INVALID-SUBSCRIPTION))
    )
    ;; Verify the caller is the subscriber
    (asserts! (is-eq tx-sender (get subscriber subscription)) ERR-NOT-AUTHORIZED)
    ;; Check if payment is due
    (asserts! (is-ok payment-due-result) ERR-PAYMENT-NOT-DUE)
    
    ;; Process payment based on token type
    (match (get token-contract subscription)
      token-principal (process-token-payment (get recipient subscription) (get payment-amount subscription) token-principal)
      ;; STX payment if no token contract
      (process-stx-payment (get recipient subscription) (get payment-amount subscription))
    )
    
    ;; Update subscription and record payment
    (unwrap! (update-subscription-after-payment subscription-id) ERR-PAYMENT-FAILED)
    (unwrap! (record-payment subscription-id "completed") ERR-PAYMENT-FAILED)
  )
)

;; Process automated payment for a subscription
(define-public (process-auto-payment (subscription-id uint))
  (let
    (
      (subscription (unwrap! (map-get? active-subscriptions { subscription-id: subscription-id }) ERR-INVALID-SUBSCRIPTION))
      (user (get subscriber subscription))
      (settings (get-auto-payment-settings user))
      (payment-amount (get payment-amount subscription))
      (payment-due-result (is-payment-due subscription-id))
    )
    ;; Check if auto-payment is enabled
    (asserts! (get enabled settings) ERR-AUTO-PAYMENT-DISABLED)
    ;; Check if payment is due
    (asserts! (is-ok payment-due-result) ERR-PAYMENT-NOT-DUE)
    ;; Check if amount is within threshold
    (asserts! (is-ok (check-payment-threshold user payment-amount)) ERR-AMOUNT-EXCEEDS-THRESHOLD)
    
    ;; Process payment based on token type
    (match (get token-contract subscription)
      token-principal (process-token-payment (get recipient subscription) payment-amount token-principal)
      ;; STX payment if no token contract
      (process-stx-payment (get recipient subscription) payment-amount)
    )
...
(define-public (approve-payment (subscription-id uint) (payment-id uint))
  (let
    (
      (approval (unwrap! (map-get? pending-approvals { subscription-id: subscription-id, payment-id: payment-id }) ERR-INVALID-PARAMETER))
      (subscription (unwrap! (map-get? active-subscriptions { subscription-id: subscription-id }) ERR-INVALID-SUBSCRIPTION))
    )
    ;; Verify the caller is the subscriber
    (asserts! (is-eq tx-sender (get user approval)) ERR-NOT-AUTHORIZED)
    
    ;; Update approval status
    (map-set pending-approvals
      { subscription-id: subscription-id, payment-id: payment-id }
      (merge approval { approved: true })
    )
    
    ;; Process payment based on token type
    (match (get token-contract subscription)
      token-principal (process-token-payment (get recipient subscription) (get amount approval) token-principal)
      ;; STX payment if no token contract
      (process-stx-payment (get recipient subscription) (get amount approval))
    )
...
(define-public (update-subscription-amount (subscription-id uint) (new-payment-amount uint))
  (let
    (
      (subscription (unwrap! (map-get? active-subscriptions { subscription-id: subscription-id }) ERR-INVALID-SUBSCRIPTION))
    )
...
(define-public (update-subscription-period (subscription-id uint) (new-payment-period uint))
  (let
    (
      (subscription (unwrap! (map-get? active-subscriptions { subscription-id: subscription-id }) ERR-INVALID-SUBSCRIPTION))
    )
